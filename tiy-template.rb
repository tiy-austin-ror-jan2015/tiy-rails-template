VERSION = 'v1.0.0'
#heroku gems
gem 'puma'
gem 'rails_12factor'
#

gem 'bootstrap-sass'

gem_group :test, :development do
  gem 'faker'

  if yes?('Would you like to use the better errors gem?')
    gem 'better_errors'
    gem 'binding_of_caller'
  end
end

if yes?('Would you like to use Simple Form?')
  gem 'simple_form'
  generate('simple_form:install --bootstrap')
end

puts 'Creating application.scss'
file 'app/assets/stylesheets/application.scss', <<-CODE
@import 'bootstrap-sprockets';
@import 'bootstrap';
CODE
puts 'Linking to bootstrap files'
run 'rm app/assest/stylesheets/application.css'
puts 'Removing old application.css file'

after_bundle do
  if yes('Would you like to add everything to git?')
    git :init
    git add: '--all'
    git commit: "-a -m 'Initial Commit [Generated by TIY Rails Template (#{version})]'"

    if yes('Would you like to push your git repo to github?')
      run('hub create')
      git push: '-u origin master'
      if yes('Would you like to open your newly created repo on github?')
        remote = `git remote -v`
        unless remote.empty?
          url_pieces = remote.scan(/github\.com:(.*)\.git/)
          url = "https://github.com/#{url_pieces.first.first}"
          run("open #{url}")
        end
      end
    end
  end
end
